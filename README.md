...
Reference: ICON Website. *About ICON*. https://www.icon-model.org
...
The core question of this project is: Can we predict which tests are likely to fail, based only on their configuration metadata, before they are even run?
...
2. Learns patterns — trains classifiers to associate configuration metadata with failure probability.
...
The parser works with real ICON output without modification. To train the model on real data in the future, simply replace `data/icon_test_data.csv` with a CSV produced by `src/parse_check_icon.py` on real `make check-icon` logs. No code changes are required.
...
Step 3 — Generate the results figure:

python3 plot_results.py

Step 4 — Predict for a new configuration:
...
Reza Kahndan
GitHub: https://github.com/rsreza
